...
try:
    ...
    pH = instrument.read_register(3, functioncode=4, signed=True) * 0.1
    nitrogênio = (instrument.read_register(4, functioncode=4, signed=True) * 0.01) + 0.0
    fósforo = (instrument.read_register(5, functioncode=4, signed=True) * 0.01) + 0.0
    potássio = (instrument.read_register(6, functioncode=4, signed=True) * 0.01) + 0.0
